...
1. If a product has a complete `spec/<product>/` pack, use that pack as the source of truth together with `AGENTS.md`.
2. If a product does not yet have a complete `spec/<product>/` pack, use the legacy Copilot-native content:
...
|-------------------|---------|------------------|
| "Defender for Cloud", "MDC", "protect my cloud", "CSPM", "server protection", "cloud security posture" | MDC | `AGENTS.md` + `spec/mdc/` |
...
2. If the product has a full `spec/` pack, load all five files from that folder before responding.
